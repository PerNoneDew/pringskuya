...
import { PaymentMethod, EventBooking } from '../../lib/types';
...
export function EventPaymentModal({
  ...
  const { paymentConfig, recordEventPayment } = useBooking();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedMethod !== 'counter' && !reference) {
      alert('Please enter the GCash/Maya account number');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            ...
            <p className="text-sm text-gray-600 mb-1">
              <span className="font-medium">Type:</span> {event.eventType.charAt(0).toUpperCase() + event.eventType.slice(1)}
            </p>
            <p className="text-sm text-gray-600 mb-1">
              <span className="font-medium">Host:</span> {event.guestName}
            </p>
            <p className="text-sm text-gray-600 mb-1">
            ...
            <div className="border-t border-purple-200 mt-3 pt-3">
              <p className="text-lg font-bold text-gray-800">
                Total Amount: ₱{event.totalPrice.toLocaleString()}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-gray-800">Select Payment Method for Staff to Accept:</h3>
            ...
            {(paymentConfig.gcashNumber || paymentConfig.mayaNumber) && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-semibold text-green-600 uppercase px-2 bg-green-50 rounded-full">Online Payment</span>
                  <div className="flex-1 h-px bg-gray-300"></div>
                </div>
                ...
                {paymentConfig.gcashNumber && (
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-sm text-gray-600">Account: <span className="font-mono text-gray-800">{paymentConfig.gcashNumber}</span></p>
                      <p className="text-xs text-amber-600 mt-1">Requires proof screenshot</p>
                    </div>
                  </button>
                )}
                ...
                {paymentConfig.mayaNumber && (
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-sm text-gray-600">Account: <span className="font-mono text-gray-800">{paymentConfig.mayaNumber}</span></p>
                      <p className="text-xs text-amber-600 mt-1">Requires proof screenshot</p>
                    </div>
                  </button>
                )}
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 mb-3 mt-4">
                <div className="flex-1 h-px bg-gray-300"></div>
                <span className="text-xs font-semibold text-blue-600 uppercase px-2 bg-blue-50 rounded-full">Cash Payment</span>
                <div className="flex-1 h-px bg-gray-300"></div>
              </div>
              ...
              >
                ...
                <div className="text-left">
                  <p className="font-semibold text-gray-800">Cash at Venue</p>
                  <p className="text-sm text-gray-600">Guest pays cash at the event venue</p>
                  <p className="text-xs text-blue-600 mt-1">Immediate confirmation</p>
                </div>
              </button>
            </div>
          </div>
            ...
            <div className={`p-4 rounded-lg border-2 space-y-4 ${
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {selectedMethod === 'gcash' ? 'GCash' : 'Maya'} Account Number
                </label>
                <input
                  type="text"
                  value={reference}
              ...
              {screenshotPreview && (
                <div className="bg-white p-3 rounded-lg border-2 border-green-300">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-medium text-gray-700">✓ Proof Uploaded</p>
          ...
          <div className="bg-green-50 p-3 rounded-lg border border-green-200 text-xs text-green-800">
            <p className="font-semibold mb-1">✓ Event Acceptance</p>
            <p>By accepting this event and recording payment, the event status will change to "Confirmed" and the guest will be notified.</p>
          </div>
