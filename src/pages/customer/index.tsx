...
export default function CustomerPage() {
  ...
  const handleBookRoom = (roomId: string) => {
    ...
    if (room.status === 'occupied') {
      showErrorNotification({
        title: 'Room Occupied',
        description: `Room ${room.roomNumber} is currently occupied. Please choose another room.`,
      });
      return;
    }
              ...
              <div className="space-y-3">
                {myBookings.map((booking) => (
                  <div key={booking.id} className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-semibold text-gray-800">Room {booking.roomNumber}</p>
                        <p className="text-sm text-gray-600">{booking.checkInDate} to {booking.checkOutDate}</p>
                        <p className="text-sm text-gray-600">{booking.numberOfGuests} guest{booking.numberOfGuests > 1 ? 's' : ''}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-bold text-gray-800">PHP {booking.totalPrice}</p>
                        <p className="text-xs text-gray-500 mb-3">
                          {booking.status.charAt(0).toUpperCase() + booking.status.slice(1)}
                        </p>
                        <div className="flex gap-2 justify-end">
                          <button
                            onClick={() => handleViewReservation(booking.id)}
                ...
                {myEventBookings.map((booking) => (
                  <div key={booking.id} className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-semibold text-gray-800">{booking.eventName || 'Event Booking'}</p>
